...
import { reportDetailByQuarter } from '../constants/index';
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex flex-col items-center py-6">
        <p className="text-yellow text-4xl font-play font-bold py-2">
          REPORT DETAIL: {reportDetailByQuarter.numberOfQuarter} - {reportDetailByQuarter.year}
        </p>
        <Decorate />
      </div>
      ...
      <div className="flex items-center justify-center py-6">
        <div className="grid grid-cols-3 gap-6 w-3/4 text-center text-lg font-play font-bold">
          <div className="shadow-lg bg-white p-4 rounded-lg">
            <p>Quarter-Year:</p>
            <span className="text-yellow text-xl">{reportDetailByQuarter.numberOfQuarter}-{ reportDetailByQuarter.year }</span>
          </div>
          <div className="shadow-lg bg-white p-4 rounded-lg">
            <p>Branch Name:</p>
            <span className="text-green text-xl">{reportDetailByQuarter.branchName}</span>
          </div>
          <div className="shadow-lg bg-white p-4 rounded-lg">
            <p>Total Revenue:</p>
            <span className="text-blue-500 text-xl">
              {Number(reportDetailByQuarter.totalRevenue).toLocaleString()} VNĐ
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-center py-6 w-full">
        <table className="w-11/12 text-center font-play shadow-lg bg-white rounded-lg">
          ...
          <tbody className="text-gray-800">
            {reportDetailByQuarter.details && reportDetailByQuarter.details.length > 0 ? (
              reportDetailByQuarter.details.map((detail, index) => (
